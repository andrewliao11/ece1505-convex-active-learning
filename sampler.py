...
class CVXSampler(Sampler):
    def __init__(self, X, labeled_mask, params):
...
class CVXSamplerNoConfidence(Sampler):
    def __init__(self, X, labeled_mask, params):
        Sampler.__init__(self, X, labeled_mask)
        self.K = params.K          # Number of classes
        self.sigma = params.sigma
    ...
    def sample(self, n, **kwargs):
        ...
        q = 2         # q-norm
        objective_fn = cp.atoms.affine.trace.trace(D.T @ Z) + self.alpha * sum(cp.atoms.norm(Z, q, axis=1))
        constraints = [Z >= 0, ones.T @ Z == ones.T]
        prob = cp.Problem(cp.Minimize(objective_fn), constraints)
        t1 = time.time()
